...
function getevent()
{
       var day = $('.day-contents').html();
		...
		if(last_arrhref == "race_card") //processing for page race_card
		{
            
			//test event day
            var Coutry = $("#ChooseCountry").val();
            $.ajax({
                        type:"POST",
		...
		{
			if(last_arrhref == "race_result") //processing for page race_result
			{

                var Coutry = $("#ChooseCountry").val();
                $.ajax({
                            type:"POST",
                            url: "home_controller/loadRaceResul_Date",
                            dataType:"text",
                            data: { Country: Coutry},
                            cache:false,
                            success:function (data) {     
...
{	
	...
	$('.day-contents').bind("click",function()
    {
			...
			/*
			|processing for each different page
			*/
			var texthref = $(location).attr('href');
			var arrhref = texthref.split('/');
			var last_arrhref = arrhref[arrhref.length-1];
			...
			{					         
                    getevent();
                     // xu ly sự kiện ngày nào có sẽ load danh sách race card lên
                    ...
                    $("#gotoearly").attr('onclick','GotoET("'+ getdate +'","'+country+'")');
                    $('#panel2').hide();
                    $('#panel1').show();
			...
			else
			{
					...
					{
						getevent();
                        // xu ly sự kiện ngày nào có sẽ load danh sách race result lên
                         var country = $("#ChooseCountry").val();
                         $.ajax({
                            type:"POST",
                            url: "home_controller/getlistraceresultday",
                            dataType:"text",
                            async: false,
                            data: { getdate: getdate,
                                    country:country
                            ...
                            cache:false,
                            success:function (data) {
                              listracesultday_Complete(data);     
                            }
                          });
					}
                    else
                    {
                        if(last_arrhref == "gen_statement") //processing for page gen_statement super_admin
					    {
					        getselecteddate1();
                        }
                    }					
    ...
    if(sRes.RaceDate != null || sRes.RaceDate !=""){
        ...
        if(info.length >0){
            for(var i=0;i<info.length;i++){                 
                str = str + '<tr>'; 
                if (info[i].Meter != null && info[i].Meter != "" )
                {
                   str = str +'<td><a href=\'javascript:void(0);\' id=\'RaceCardComple\' onclick=\'RaceCardDetail("'+ info[i].RaceID +'")\'>'+ info[i].RaceID +"M"+info[i].Meter+'</a></td>';  
                }
                else
                {
