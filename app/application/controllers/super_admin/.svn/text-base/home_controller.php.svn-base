...
class Home_controller extends CI_Controller {
	...
	*/
	public function __construct()
	{
        ...
        $this->load->model('superadmin/m_feedback');
    ...
    public function feedback_customer()
    {
        if(isset($_SESSION['AccUserSuper']) && ($_SESSION['AccUserSuper']['level'] ==1 || $_SESSION['AccUserSuper']['level'] == 2))
        {
            $p_arr  =   array(
                                ...
                                'active'            =>  9,
                                'p_custom_css'      =>  $this->load->view('super_admin/css/css_index_view', '', TRUE),
                                'p_custom_js'       =>  $this->load->view('super_admin/js/js_feedback_view', '', TRUE)
                            );
            /*
            |----------------------------------------------------------------
            | Load Head View
            ...
            */
            ...
            $this->load->view("super_admin/header_view");
            $this->load->view("super_admin/left_view");
            $this->load->view("super_admin/feed_back_view");
            $this->load->view("super_admin/footer_view");
	...
	* Function view add agent
	* Date: 24/04/2015
	* URL Page: super_admin/sell_phone
	* Rewrite routing: file config/routes.php
	* @param 
	* @return Show Add agent
	*/
	public function sell_phone()
	{	
        if(isset($_SESSION['AccUserSuper']) && $_SESSION['AccUserSuper']['level'] ==1)
        {
            $p_arr 	=	array(
    						...
    						'active'	=>	8,
                            'p_custom_css'		=>	$this->load->view('super_admin/css/css_index_view', '', TRUE),
                            'p_custom_js' 		=>	$this->load->view('super_admin/js/js_sell_phone_view', '', TRUE)
    					);
    		/*
    		|----------------------------------------------------------------
    		| Load Head View
    		...
    		*/
    		...
    		$this->load->view("super_admin/header_view");
    		$this->load->view("super_admin/left_view");
    		$this->load->view("super_admin/sell_phone_view");
    		$this->load->view("super_admin/footer_view");
        }
		else
            header('Location: '.base_url('super_admin/login'));
	...
	*/
	public function agent_page()
	{
	   if(isset($_GET['id']))
       {
            ...
            if(isset($_SESSION['AccUserSuper']) && $_SESSION['AccUserSuper']['level'] ==1 && $agent->level==4)
                $role=1;
            $p_arr_active=0;
            $parent = $this->m_agent_page->get_parent_by_id($agent->pid);
    		...
    		if(count($agent)==1)
    			$arr_res=array("user"=>$agent,"active_member"=>$active_member,"inactive_member"=>$inactive_member,"role"=>$role,"parent"=>$parent);
    		else
    ...
    /**
	* Function view sell phone page
	* Date: 24/04/2015
	* URL Page: super_admin/sell_phone_page
	* Rewrite routing: file config/routes.php
	* @param 
	* @return Show sellphone page
	*/
	public function sell_phone_page()
	{
	   if(isset($_GET['id']))
       {
            $id                 =   $_GET['id'];
    		$sellphone          =   $this->m_sell_phone_page->get_sell_phone_by_id($id);
            
            $p_arr 	=	array(
    						'title'		         =>	'Super admin page',
    						'active'	         =>	8,
    						//'p_custom_css'		 =>	$this->load->view('super_admin/css/css_add_new_user_view', '', TRUE),
    						'p_custom_js' 		 =>	$this->load->view('super_admin/js/js_sell_phone_page_view', '', TRUE)
    					);
    		
    		if(count($sellphone) == 1)
    			$arr_res=array("sellphone"   =>  $sellphone);
    		else
    			header('Location: dashboard');
    		/*
    		|----------------------------------------------------------------
    		...
    		*/
    		...
    		$this->load->view("super_admin/header_view");
    		$this->load->view("super_admin/left_view");
    		$this->load->view("super_admin/sellphone_page_view",$arr_res);
    		$this->load->view("super_admin/footer_view");
       }
	}
	...
	*/
    ...
    {
        $req = $this->m_feedback->load_list_feedback();
        echo json_encode($req);
    }
    ...
    */
    public function btnsave_sellphone()
    {
        if(isset($_SESSION['AccUserSuper']) && $_SESSION['AccUserSuper']['level'] ==1)
        {
            $req = $this->m_sell_phone_page->btnsave_sellphone();
            echo json_encode($req);
        }
		else
            header('Location: '.base_url('super_admin/login'));   
    ...
    */
    public function btnsaveedit_sellphone()
    {
        if(isset($_SESSION['AccUserSuper']) && $_SESSION['AccUserSuper']['level'] ==1)
        {
            $req = $this->m_sell_phone_page->btnsaveedit_sellphone();
            echo json_encode($req);
        }
		else
            header('Location: '.base_url('super_admin/login'));   
    ...
    */
    public function get_list_sellphone()
    {
        if(isset($_SESSION['AccUserSuper']) && $_SESSION['AccUserSuper']['level'] ==1)
        {
            $req = $this->m_sell_phone_page->get_list_sellphone();
            echo json_encode($req);
        }
		else
            header('Location: '.base_url('super_admin/login'));   
    ...
    */
    public function btndel_sellphone()
    {
        if(isset($_SESSION['AccUserSuper']) && $_SESSION['AccUserSuper']['level'] ==1)
        {
            $req = $this->m_sell_phone_page->btndel_sellphone();
            echo json_encode($req);
        }
		else
            header('Location: '.base_url('super_admin/login'));   
    ...
    public function SaveParentContact(){
       $req = $this->m_agent_page->SaveParentContact();
       echo json_encode($req); 
    }
    ...
    */
    public function showlogagent()
    {
        if(isset($_SESSION['AccUserSuper']) && $_SESSION['AccUserSuper']['level'] ==1)
        {
            $req = $this->m_gen_statement->showlogagent();
            echo json_encode($req);
        }
		else
            header('Location: '.base_url('super_admin/login'));   
    ...
    */
    public function btnsaverefund()
    {
        if(isset($_SESSION['AccUserSuper']) && $_SESSION['AccUserSuper']['level'] ==1)
        {
            $req = $this->m_gen_statement->btnsaverefund();
            echo json_encode($req);
        }
		else
            header('Location: '.base_url('super_admin/login'));   
    ...
    */
    public function btnpayall()
    {
        if(isset($_SESSION['AccUserSuper']) && $_SESSION['AccUserSuper']['level'] ==1)
        {
            $req = $this->m_gen_statement->btnpayall();
            echo json_encode($req);
        }
		else
            header('Location: '.base_url('super_admin/login'));   
    ...
    */
    public function btnpayorder()
    {
        if(isset($_SESSION['AccUserSuper']) && $_SESSION['AccUserSuper']['level'] ==1)
        {
            $req = $this->m_gen_statement->btnpayorder();
            echo json_encode($req);
        }
		else
            header('Location: '.base_url('super_admin/login'));   
