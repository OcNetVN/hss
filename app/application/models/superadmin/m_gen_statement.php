...
class M_gen_statement extends CI_Model{
            ...
            foreach($list_agent_active as $row_agent_active)
            {
                ...
                if(count($row_statement) < 1) //chua co luu du lieu 10/03 cua agent trong bang statement
                {
                    ...
                    if(count($row_statement) > 0) //co du lieu cua thang 02
                    {
                        ...
                        //$re_refund                  = (int)($row_statement->Refund);
        ...
        if(isset($_POST['page']) && isset($_POST['date']))
        {
            ...
            if($totalrow    > 0)
            {
                ...
                foreach($list_agent_active as $row_agent_active)
                {
                        ...
                        {   
                            $sql_tran                       =   "SELECT * FROM `usertrans` 
                                                                    WHERE `agent` = '$row_agent_active->id' AND `Action` = 'SELLPHONE' 
                                                                    AND DATE(`CreatedDate`) >= '$year-$month-10' AND DATE(`CreatedDate`) <= '$year-$month-$day'";
                            $list_tran                       =   $this->db->query($sql_tran)->result();
                            $money_sellphone                =   0;
                            foreach($list_tran as $row_tran)
                            {
                                $money_sellphone            +=  (int)$row_tran->Amt;
                            }
                            
                            $sql_tran_refund                =   "SELECT * FROM `usertrans` 
                                                                    WHERE `agent` = '$row_agent_active->id' AND `Action` = 'REFUND' 
                                                                    AND DATE(`CreatedDate`) >= '$year-$month-10' AND DATE(`CreatedDate`) <= '$year-$month-$day'";
                            $list_tran_refund               =   $this->db->query($sql_tran_refund)->result();
                            $money_refund                   =   0;
                            foreach($list_tran_refund as $row_tran_refund)
                            {
                                $money_refund               +=  (int)$row_tran_refund->Amt;
                            }
                            ...
                            if(count($row_statement) > 0) //co du lieu cua thang 03 (show button pay)
                            {
                                ...
                                if($row_statement->Status == 1 || $row_statement->Status == "1")    //hệ thống đã tự insert vào db
                                {
									...
									if(count($list_usertrans) > 0) //da co thanh toan sau ngay 10/03 va truoc ngay hien tai dc chon
                                    {
										...
										$sql_usertrans_sellphone        = "SELECT * FROM `usertrans` WHERE `CreatedDate` > '$datepay_last' AND (`Action` = 'SELLPHONE') AND `agent` = '$row_agent_active->id'";
                                        //echo $sql_usertrans;
                                        $query_usertrans_sellphone      = $this->db->query($sql_usertrans_sellphone)->result();
                                        
                                        $total_money_usertrans_sellphone= 0;
                                        foreach($query_usertrans_sellphone as $row_usertran_sellphone)
                                        {
                                            $total_money_usertrans_sellphone      += (int)$row_usertran_sellphone->Amt;
                                        } 
										
                                        $tax_usertrans                  = $total_money_usertrans*$percenttax/100;
                                        $revious_balance                = (int)$row_statement->HaveToPay - $total_pay - $total_money_usertrans - $tax_usertrans - $total_money_usertrans_sellphone;
                        ...
                        else
                        {
                            ...
                            { 
                                $sql_tran                       =   "SELECT * FROM `usertrans` 
                                                                        WHERE `agent` = '$row_agent_active->id' AND `Action` = 'SELLPHONE' 
                                                                        AND DATE(`CreatedDate`) >= '$yearre-$monthre-10' AND DATE(`CreatedDate`) <= '$year-$month-$day'";
                                $list_tran                       =   $this->db->query($sql_tran)->result();
                                $money_sellphone                =   0;
                                foreach($list_tran as $row_tran)
                                {
                        ...
                        if($flag_show == 1)
                        {
                            $str_list .= '<table class="table table-hover table-bordered">';
                                $str_list .= '<tbody>';
                                    ...
                                    $str_list .= '<tr>';
                                        $str_list .= '<td>'.$this->lang->line(LANG_SELL_PHONE).':</td>';
                                        $str_list .= '<td></td>';
                                        $str_list .= '<td>'.$row_agent_active->currency.number_format($money_sellphone,2).'</td>';
                                    $str_list .= '</tr>';
                                    
                                    $str_list .= '<tr>';
                                        $str_list .= '<td>'.$this->lang->line(LANG_REFUND).':</td>';
                                        $str_list .= '<td></td>';
                                        $str_list .= '<td>'.$row_agent_active->currency.number_format($money_refund,2).'</td>';
                                    $str_list .= '</tr>';
                                    ...
                                    $str_list .= '<tr>';
                                        ...
                                        $str_list .= '<td><h4>'.$row_agent_active->currency.number_format(($totalbalance+$money_sellphone+$money_refund),2).'</h4></td>';
                                    $str_list .= '</tr>';
                                    ...
                                    $str_list .= '<tr>';
                                        $str_list .=    '<td class="text-right" colspan="3">
                                                            <button type="button" class="btn btn-warning" id="btnshowlog" onclick="showlogagent(\''.$row_agent_active->id.'\',\''.$yyyy_mm_dd.'\');" data-toggle="modal" data-target="#modalshowlog">'.$this->lang->line(LANG_SHOW_LOG).'</button>';
                                        if(($totalbalance+$money_sellphone) > 0)
                                        $str_list .=    '   <button type="button" class="btn btn-warning" id="btnshowlog" onclick="refund(\''.$row_agent_active->id.'\',\''.$yyyy_mm_dd.'\');" data-toggle="modal" data-target="#modalrefund">'.$this->lang->line(LANG_REFUND).'</button>';
                                        $str_list .=    '   </td>';
                                    $str_list .= '</tr>';
                                    $str_list .= '<tr>';
    ...
    */
    public function get_total_active_1_15_by_agentid_from_to($agentid,$month1,$year1,$month2,$year2) //yyyy-mm-dd
    {
        $sql        = "SELECT COUNT(*) AS total FROM `usertrans` 
                    	WHERE `agent` = '$agentid' AND `Action` = 'ACTIVE' AND `CreatedDate` BETWEEN '$year1-$month1-01 00:00:01' 
    ...
    */
    public function get_active_01_15_by_date_after_10th($agentid,$day,$month,$year)
    {
        ...
        $sql        = "SELECT COUNT(*) AS total FROM `usertrans` 
                	WHERE `agent` = '$agentid' AND `Action` = 'ACTIVE' AND `Amt` > 0 AND SUBSTRING(`CreatedDate`,1,7) = '$year-$month' AND 
    ...
    public function get_active_01_15_by_date_after_10th_3($agentid,$day,$month,$year,$day2,$month2,$year2)
    {
        $rowid      =   $this->get_rowid_by_userid($agentid);
        $sql        = "SELECT COUNT(*) AS total FROM `usertrans` 
                	WHERE `agent` = '$agentid' AND `Action` = 'ACTIVE' AND `Amt` > 0 AND (SUBSTRING(`CreatedDate`,1,7) = '$year-$month' OR SUBSTRING(`CreatedDate`,1,7) = '$year2-$month2') AND 
                		SUBSTRING(`CreatedDate`,9,2) >= 10 AND SUBSTRING(`CreatedDate`,9,2) < 16 AND SUBSTRING(`CreatedDate`,1,10) >= '$year-$month-$day' AND SUBSTRING(`CreatedDate`,1,10) <= '$year2-$month2-$day2'";
                        //echo $sql;die;
        $query      = $this->db->query($sql)->row();
        return      $query;
    ...
    */
    public function btnpayall()
    {
        if(isset($_POST['agentid']) && isset($_POST['yyyy_mm_dd']))
        {
            ...
            {
                $date                   = $str_year_1."-".$str_month_1."-".$day;
            }
    ...
    /*
    |----------------------------------------
    |function show log of agent
    |----------------------------------------
    */
    public function showlogagent()
    {
        $str_res                    =   "";
        $stt                        =   0;
        $notify                     =   "";
        ...
        $agentid                    =   $_POST["agentid"];
        $yyyy_mm_dd                 =   $_POST["yyyy_mm_dd"];   //yyyy-mm-dd
        $day                        =   substr($yyyy_mm_dd,-2);
        $month                      =   substr($yyyy_mm_dd,5,2);
        $year                       =   substr($yyyy_mm_dd,0,4);
        ...
        if($day >= 10)
        {
            $sql                    =   "SELECT * FROM `usertrans` WHERE `agent` = '$agentid' AND DATE(`CreatedDate`) >= '$year-$month-10' AND DATE(`CreatedDate`) <= '$year-$month-$day' ORDER BY `CreatedDate` DESC";
        }   
        else
        {
            $sql                    =   "SELECT * FROM `usertrans` WHERE `agent` = '$agentid' AND DATE(`CreatedDate`) >= '$yearre-$monthre-10' AND DATE(`CreatedDate`) <= '$year-$month-$day' ORDER BY `CreatedDate` DESC";
        }   
        $list_usertran              =   $this->db->query($sql)->result();
        
        if(count($list_usertran) > 0)
        {
            $str_res                .=   '<table class="table table-hover table-bordered" id="tbllistusertrans">
                                              <thead>
                                              <tr>
                                                  <th>#</th>
                                                  <th>'.$this->lang->line(LANG_TR_SERIAL).'</th>
												  <th>'.$this->lang->line(LANG_TR_IMEI).'</th>
                                                  <th>'.$this->lang->line(LANG_TR_ACTION).'</th>
                                                  <th>'.$this->lang->line(LANG_MONEY).'</th>
                                                  <th>'.$this->lang->line(LANG_TR_CREATE_DATE).'</th>
                                              </tr>
                                              </thead>
                                              <tbody id="tbodylistusertrans">';
                                              
            $stt                    =   1;
            foreach($list_usertran as $row_usertran)
            {
                $sql_user           =   "SELECT * FROM `users` WHERE `rowid` = '$row_usertran->userid'";
				//echo $sql_user;die;
                $usertran           =   $this->db->query($sql_user)->row();
                if(count($usertran) > 0)
				{
					$serial			=	$usertran->serialNo;
					$imei           =   $usertran->ImieNo;
				}
                else
				{
					$serial			=	"";
                ...
                if($row_usertran->Action == "SELLPHONE")
                {
                    $imei           =   $row_usertran->userid;
                }
                $str_res            .=   '<tr>
                                                  <td>'.$stt.'</td>
                                                  <td>'.$imei.'</td>
												  <td>'.$serial.'</td>
                                                  <td>'.$row_usertran->Action.'</td>
                                                  <td>'.$row_usertran->Amt.'</td>
                                                  <td>'.$row_usertran->CreatedDate.'</td>
                                              </tr>';
                $stt++;
            }                                 
            $str_res                .=  '</tbody>
    ...
    /*
    |----------------------------------------
    |function click btn save refund
    |----------------------------------------
    */
    public function btnsaverefund()
    {
        if(isset($_POST['agentid']) && isset($_POST['yyyy_mm_dd']))
        {
            $agentid                    = $_POST['agentid'];
            ...
            if(count($row_statement) > 0)
            {
                ...
                $refund                 =   $row_statement->Refund;
                $havetopay              =   $row_statement->HaveToPay;
                $pay                    =   (int)($refund + $money);
                
                $havetopay_new          =   $havetopay - $money;
                $minusmoney             =   -(int)$money;
                $this->db->trans_start();
                $sql                    =   "UPDATE `statement` SET `Refund` = '$pay', `HaveToPay` = '$havetopay_new' WHERE `UserID` = '$agentid' AND SUBSTRING(`FixDate`,6,2)= '$month' AND SUBSTRING(`FixDate`,1,4)= '$year'";
                //echo $sql;die;
                $query                  =   $this->db->query($sql);
                
                $sql_usertrans          =   "INSERT INTO `usertrans` 
                                                    (`id`, `userid`, `Action`, `Value`, `Amt`,          `CreatedDate`, `CreatedBy`, `agent`) 
                                            VALUES (NULL, '$rowid',   'REFUND', '1',    '$minusmoney',  '$datepay',       'admin',  '$agentid')";
                $query_usertrans        =   $this->db->query($sql_usertrans);
