        ...
        if(isset($_POST['page']) && isset($_POST['date']))
        {
            ...
            if($totalrow    > 0)
            {
                ...
                {    
                        ...
                        if((int)$day >= 10) //ngay >= 10/03 lay statemnt cua thang 04
                        {
                            if(strlen($month)       == 2)
                            {
                            ...
                            if(count($row_statement) > 0) //co du lieu cua thang 04
                            {
                                /*$FixDate                    = $row_statement->FixDate;
                                $arr_fixday                 = explode(" ",$FixDate);
                                $arr_fixday1                = explode("-",$arr_fixday[0]);
                                $month_fixday               = (int)$arr_fixday1[1];
                                $year_fixday                = (int)$arr_fixday1[0];
                                $str_month_year_fixday      = $month_fixday.$year_fixday;*/
                                
                                $revious_balance            = $row_statement->PreBalance;
                                $activeuser                 = $row_statement->ActiveUser;
                                $pay                        = $row_statement->Pay;
                                $newmember_1_15             = $this->get_total_active_or_inactive_customer_by_agentid_status_createdate($row_agent_active->id,1,$arr_date[1],$arr_date[2],1)->total;
                                $newmember_16_31            = $this->get_total_active_or_inactive_customer_by_agentid_status_createdate($row_agent_active->id,1,$arr_date[1],$arr_date[2],2)->total;
                            }
                            else //khong co du lieu cua thang 04 (phai tinh)
                            {
                                $row_statement          = $this->get_tbl_stament_by_userid($row_agent_active->id);
                                if(count($row_statement) > 0) //co du lieu cua kì truoc đó
                                {
                                    $revious_balance            = (int)($row_statement->PreBalance)+((int)($row_statement->HaveToPay)-(int)($row_statement->Pay));
                        ...
                        else //ngay < tu ngay 01/03 den 09/03 lay statemnt cua thang 03
                        {
                            $row_statement          = $this->get_tbl_stament_by_userid_month($row_agent_active->id,$arr_date[1],$year); //lay du lieu cua thang 03
                            if(count($row_statement) > 0) //co du lieu cua thang 03
                            {
                                $revious_balance            = $row_statement->PreBalance;
                                $activeuser                 = $row_statement->ActiveUser;
                                $pay                        = $row_statement->Pay;
                            ...
                            else //khong co du lieu cua thang 03 (phai tinh)
                            {
                                $row_statement          = $this->get_tbl_stament_by_userid($row_agent_active->id);
                                if(count($row_statement) > 0) //co du lieu cua kì truoc đó
                                {
                        ...
                        $str_list .= '<table class="table table-hover table-bordered">';
                            $str_list .= '<tbody>';
                                ...
                                $str_list .= '</tr>';
                                $str_list .= '<tr>';
                                    $str_list .= '<td>'.$this->lang->line(LANG_NEW_MEMBER1_15).':</td>';
                                    $str_list .= '<td>'.$newmember_1_15.' x '.$row_agent_active->currency.(int)$fees.'</td>';
                                    $str_list .= '<td>'.$row_agent_active->currency.number_format($newmember_1_15*((int)$fees)).'</td>';
                                $str_list .= '</tr>';
                                $str_list .= '<tr>';
                                    $str_list .= '<td>'.$this->lang->line(LANG_NEW_MEMBER16_31).':</td>';
                                    $str_list .= '<td>'.$newmember_16_31.' x '.$row_agent_active->currency.((int)((int)$fees/2)).'</td>';
                                    $str_list .= '<td>'.$row_agent_active->currency.number_format($newmember_16_31*((int)((int)$fees/2))).'</td>';
                                $str_list .= '</tr>';
                                $str_list .= '<tr>';
                                    $str_list .= '<td colspan="2"><h4>'.$this->lang->line(LANG_TOTAL).'</h4></td>';
                                        $totalmoney = $revious_balance+$activeuser*(int)$fees+$newmember_1_15*(int)$fees+$newmember_16_31*((int)((int)$fees/2));
                                ...
                                $str_list .= '<tr>';
                                    $str_list .= '<td colspan="2"><h4>'.$this->lang->line(LANG_REMAINDER_AMOUNT).'</h4></td>';
                                        $remainder_amount = $revious_balance+$activeuser*(int)$fees+$newmember_1_15*(int)$fees+$newmember_16_31*((int)((int)$fees/2)) - $pay;
    ...
    public function get_list_tbl_user($level,$status,$start,$limit)
    {
        if(isset($_SESSION['AccUserSuper']))
        {
            if($_SESSION['AccUserSuper']['level']==1)
                $sql="SELECT * FROM `users` WHERE `level` = $level AND `status`=$status ORDER BY `rowid` LIMIT $start,$limit";
            else
            {
                $id=$_SESSION['AccUserSuper']['id'];
                $sql="SELECT * FROM `users` WHERE `level` = $level AND `status`=$status AND `pid`='$id' ORDER BY `rowid` LIMIT $start,$limit";
            }
        }
        else
            $sql="SELECT * FROM `users` WHERE `level` = $level AND `status`=$status ORDER BY `rowid` LIMIT $start,$limit";
