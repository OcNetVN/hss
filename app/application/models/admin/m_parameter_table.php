...
class M_parameter_table extends CI_Model{
        ...
        public function loadLeagueTable()
        {
            ...
            $sql_country = "SELECT * FROM `scc_all_tables` WHERE `id`='$cate'";
            $ar_country  = $this->db->query($sql_country)->result();
            $arr = array('SubCate' => $arr_SubCate,'Country'=>$ar_country);
            return $arr;
        }
     ...
     public function deleteLeagueTable(){
      ...
      public function SaveCountry()
      {
          ...
          //{
              if($id != null)
              {
                  ...
                  if(count($Cate) != 0)
                  {
                      $sql_update = "UPDATE `scc_all_tables` SET `CountryName`='$name',`CountryName_CN`='$name_cn',`Flag`='$filename' WHERE `id`='$id'";
                  ...
                  {
                      $sql = "INSERT INTO `scc_all_tables`(`CountryName`,`CountryName_CN`,`Flag`,`CreadedDate`)VALUES('$name','$name_cn','$filename',NOW())";
      ...
      public function addLeagueTable(){
          ...
          if($id != null)
          {
              ...
              if(count($Cate) != 0)
              {
                  $sql_update = "UPDATE `scc_league_tables` SET `League`='$league',`League_cn`='$league_cn',`Venue`='',`Season`='' WHERE `id`='$id'";
                  $result = $this->db->query($sql_update);
                  if($result)
                    $req = 1;
              }
              else{
                  $sql = "INSERT INTO `scc_league_tables`(`League`,`League_cn`,`Venue`,`Season`,`RefIdFlag`,`CreadedDay`)VALUES('$league','$league_cn','','','$idparent',NOW())";
                  $result = $this->db->query($sql);
                  if($result)
                    $req = 1;
              }  
          ...
          {
            $sql = "INSERT INTO `scc_league_tables`(`League`,`Venue`,`Season`,`RefIdFlag`,`CreadedDay`)VALUES('$league','$league_cn','','','$idparent',NOW())";
            $result = $this->db->query($sql);
            if($result)
              $req = 1;
          }
